...
type ConstructorFunc[T any] func(ctx context.Context, router adapter.Router, logger log.Factory, tag string, options T) (adapter.Provider, error)
...
func Register[Options any](registry *Registry, providerType string, constructor ConstructorFunc[Options]) {
	registry.register(providerType, func() any {
		return new(Options)
	}, func(ctx context.Context, router adapter.Router, logger log.Factory, tag string, rawOptions any) (adapter.Provider, error) {
		var options *Options
		if rawOptions != nil {
...
type (
	...
	constructorFunc        func(ctx context.Context, router adapter.Router, logger log.Factory, tag string, options any) (adapter.Provider, error)
)
...
func (r *Registry) CreateProvider(ctx context.Context, router adapter.Router, logger log.Factory, tag string, providerType string, options any) (adapter.Provider, error) {
	r.access.Lock()
	defer r.access.Unlock()
