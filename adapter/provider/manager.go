...
func (m *Manager) Providers() []adapter.Provider {
	...
	for _, provider := range m.providers {
		if manager := provider.Provider(); manager != nil {
			providers = append(providers, manager.Providers()...)
		}
	}
	return providers
}
...
func (m *Manager) Provider(tag string) (adapter.Provider, bool) {
	...
	for _, provider := range m.providers {
		if manager := provider.Provider(); manager != nil {
			provider, found = manager.Provider(tag)
			if found {
				return provider, true
			}
		}
...
func (m *Manager) Create(ctx context.Context, router adapter.Router, logger log.Factory, tag string, providerType string, options any) error {
	if tag == "" {
		return os.ErrInvalid
	}
