...
func NewRemote(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.RemoteProviderOptions) (adapter.Provider, error) {
	...
	provider := &Remote{
		myProviderAdapter: myProviderAdapter{
			...
			outboundManager: NewOutboundManger(ctx, logger),
			defaultOutbound: service.FromContext[adapter.OutboundManager](ctx),
	...
	subInfo, lastUpdatedTime, err := provider.parseCacheFile()
	if err != nil {
		return provider, nil
	}
...
func (p *Remote) PostStart() error {
	var dialer N.Dialer
	if p.detour != "" {
		outbound, loaded := p.defaultOutbound.Outbound(p.detour)
		if !loaded {
			return E.New("download_detour not found: ", p.detour)
		}
		dialer = outbound
	} else {
		dialer = p.defaultOutbound.Default()
	}
	p.dialer = dialer
	go p.loopUpdate()
