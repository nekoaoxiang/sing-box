...
func NewLocal(ctx context.Context, router adapter.Router, logger log.ContextLogger, tag string, options option.LocalProviderOptions) (adapter.Provider, error) {
	...
	outboundList, lastUpdatedTime, err := provider.parseProviderFile()
	if err != nil {
		return nil, err
	}
	...
	if outboundList == nil {
		return nil, E.New("outbounds is missing")
	}
...
func (p *Local) parseProviderFile() ([]adapter.Outbound, time.Time, error) {
	...
	outbounds_option, err := p.newParser(content)
	if err != nil {
		return nil, time.Time{}, err
	}
	...
	return outbounds_adapter, fileModeTime, err
}
